...
/* Global variables */
uint8_t global_word_length;

char global_word[CONST_WORD_LIMIT];
char global_charset[CONST_CHARSET_LIMIT];
char global_cracked[CONST_WORD_LIMIT];
...
__global__ void crack_md5_password(uint8_t wordLength, char* charsetWord, uint32_t hash01, uint32_t hash02, uint32_t hash03, uint32_t hash04){
...
int main(int argc, char* argv[]){
  ...
  /* Fill memory */
  memset(global_word, 0, CONST_WORD_LIMIT);
  memset(global_cracked, 0, CONST_WORD_LIMIT);
  memcpy(global_charset, CONST_CHARSET, CONST_CHARSET_LENGTH);
  
  /* Current word length = minimum word length */
  global_word_length = CONST_WORD_LENGTH_MIN;
  ...
  for(int device = 0; device < devices; device++){
    ...
    /* Copy to each device */
    CHECK_ERROR(cudaMemcpyToSymbol(g_deviceCharset, global_charset, sizeof(uint8_t) * CONST_CHARSET_LIMIT, 0, cudaMemcpyHostToDevice));
    CHECK_ERROR(cudaMemcpyToSymbol(g_deviceCracked, global_cracked, sizeof(uint8_t) * CONST_WORD_LIMIT, 0, cudaMemcpyHostToDevice));
  ...
  while(true){
    ...
    for(int device = 0; device < devices; device++){
      ...
      /* Copy current data */
      CHECK_ERROR(cudaMemcpy(words[device], global_word, sizeof(uint8_t) * CONST_WORD_LIMIT, cudaMemcpyHostToDevice)); 
    
      /* Start kernel */
      crack_md5_password<<<TOTAL_BLOCKS, TOTAL_THREADS>>>(global_word_length, words[device], md5Hash[0], md5Hash[1], md5Hash[2], md5Hash[3]);
      
      /* Global increment */
      result = next(&global_word_length, global_word, TOTAL_THREADS * HASHES_PER_KERNEL * TOTAL_BLOCKS);
    }
    
    /* Display progress */
    char word[CONST_WORD_LIMIT];
    
    for(int i = 0; i < global_word_length; i++){
      word[i] = global_charset[global_word[i]];
    }
    
    // std::cout << "NOTE: Currently at " << std::string(word, global_word_length) << " (" << (uint32_t)global_word_length << ")" << std::endl;
    ...
    for(int device = 0; device < devices; device++){
      ...
      /* Copy result */
      CHECK_ERROR(cudaMemcpyFromSymbol(global_cracked, g_deviceCracked, sizeof(uint8_t) * CONST_WORD_LIMIT, 0, cudaMemcpyDeviceToHost)); 
      
      /* Check result */
      if(found = *global_cracked != 0){     
        std::cout << "Password cracked: " << global_cracked << std::endl; 
        break;
      }
    }
    ...
    if(!result || found){
      if(!result && !found){
        std::cout << "No matching password found (host)" << std::endl;
      }
  ...
  std::cout << "Total Computation time " << milliseconds << " ms" << std::endl;
